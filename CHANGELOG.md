...
- ``read_file`` now sends a geopandas User-Agent when downloading from a URL, so
  servers that reject the default Python User-Agent with a 403 no longer fail (#3441).
