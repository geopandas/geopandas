...
- Fix `to_parquet`/`to_feather` (and `to_arrow`) not stripping datum ensemble
  member `id`s from the per-column Arrow field ``ARROW:extension:metadata``
  CRS (only the top-level "geo" metadata CRS was sanitized), which could
  produce a CRS unreadable by tools relying on older PROJ versions (#3804).
