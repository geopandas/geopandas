...
- Static plotting in GeoPandas has been re-implemented. The new tooling shall be mostly
  backwards compatible while bringing a set of enhancements:
    ...
    - Plotting of continuous variables with a classification scheme can now produce
      a colorbar instead of a categorical legend when using `colorbar=True` within 
      `legend_kwds`.
    - `vmin` and `vmax` keywords are supported when `scheme` is defined, not ignored. 
    - When using categorical colormap with categorical plots, colors are now assigned
      in order to optimize the contrast of the resulting map.
    - Colorbar size is automatically adjusted to be approximately the size of the plot.
    - Colorbar now indicates extensions if the `vmin` and/or `vmax` clip the values.
    - Legends are created automatically by matplotlib. This also means that you can 
      create a single legend from a layered map by calling `ax.legend()` directly.
    - GeoPandas now labels axes using the information on units from CRS. This can be
      controlled via the `add_labels` keyword.
    - Static plots can include a basemap defined by the `tiles` keyword, exposing
      `contextily.add_basemap` from within the `plot` method. Requires `contextily`
      package to be installed.
    - `scheme` keyword can take a value `"greedy"` resulting in a greedy (topological)
      coloring where no two adjacent polygons share a color. Requires `mapclassify` and 
      `libpysal` packages.
  
  The related refactoring of the plotting infrastructure has changed the internal
  representation of the figure generated by GeoPandas. This can be considered a breaking
...
- Fix the ``test_to_wkb`` test assuming little-endian WKB output and therefore
  failing on big-endian systems (e.g. s390x); the test now explicitly requests
  the byte order of the expected output (#3866).
- Ensure that points generated by `sample_points` are not sorted along
  x-axis (#3773).
- Fix `GeoSeries.sample_points` looping without end on zero-area polygons, such as a
  collapsed ring; it now returns an empty `MultiPoint`, as it already did for empty
  and missing geometries (#3851).
