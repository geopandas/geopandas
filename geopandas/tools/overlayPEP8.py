...
def overlay_intersection(df1, df2):
    ...
    if nei != []:
        ...
        pairs['Intersection'] = pairs.apply(
            lambda x: (
                x['geometry_1'].intersection(
                    x['geometry_2'])).buffer(0),
            axis=1)
        pairs = GeoDataFrame(pairs, columns=pairs.columns, crs=df1.crs)
        dfinter = pairs.drop(
            ['geometry_1', 'geometry_2', 'sidx', 'bbox'], axis=1)
        ...
        return GeoDataFrame([], columns=list(
            set(df1.columns).union(df2.columns)), crs=df1.crs)
...
def overlay_symmetric_diff(df1, df2):
    ...
    dfsym = df1.merge(
        df2, on=[
            'idx1', 'idx2'], how='outer', suffixes=[
            '_1', '_2'])
    dfsym['geometry'] = dfsym.geometry_1
    dfsym.loc[dfsym.geometry_2.isnull() == False,
              'geometry'] = dfsym.loc[dfsym.geometry_2.isnull() == False,
                                      'geometry_2']
    ...
    dfsym.drop(['bbox', 'sidx', 'idx', 'idx1',
                'idx2', 'new_g'], axis=1, inplace=True)
    dfsym = dfsym.reset_index(drop=True)
    return dfsym


def overlay(df1, df2, how='intersection', make_valid=True,
            reproject=True, use_sindex=None, **kwargs):
    """Perform spatial overlay between two polygons.
