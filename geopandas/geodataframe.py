    ...
    def iterfeatures(self, na="null", show_bbox=False):
        ...
        if len(properties_cols) > 0:
            ...
            for i, row in enumerate(properties):
                ...
                if na == "drop":
                    properties_items = {
                        k: (v if not isinstance(v,BaseGeometry) else mapping(v)) for k, v in zip(properties_cols, row) if not pd.isnull(v)
                    }
                else:
                    properties_items = {k: (v if not isinstance(v,BaseGeometry) else mapping(v)) for k, v in zip(properties_cols, row)}
