...
class TestNoSindex:
    ...
    def test_no_sindex_installed(self):
        """Check that an `ImportError` is raised when attempting to generate a spatial
        index for a GoeDataFrame when the spatial index implementation library is not
        available.  Confirm that the index is not generated by the failed attempt to
        generate it.
        """
        t1 = Polygon([(0, 0), (1, 0), (1, 1)])
        t2 = Polygon([(0, 0), (1, 1), (0, 1)])
        s = GeoSeries([t1, t2])
        d = GeoDataFrame({"geometry": [t1, t2]}, geometry="geometry")
...
class TestFrameSindex:
    def setup_method(self):
        ...
        self.df = GeoDataFrame(data, geometry="geometry")
    ...
    def test_rebuild_on_multiple_col_selection(self):
        ...
        original_index = self.df.sindex
        # Selecting a subset of columns preserves the index
        subset1 = self.df[["geometry", "A"]]
        assert subset1.sindex is original_index
        subset2 = self.df[["A", "geometry"]]
...
class TestPygeosInterface:
    def setup_method(self):
        ...
        self.df = GeoDataFrame(data, geometry="geometry")
        self.expected_size = len(data["geometry"])
