...
from geopandas._array_spherical import GeographyArray, GeometryDtype, from_spherely
...
def dtype():
    ...
    return GeometryDtype(engine="spherical")
...
def make_data():
    ...
    a[:] = [spherely.create_point(i, i) for i in range(size)]
    ga = from_spherely(a)
    return ga
...
def data_missing():
    ...
    return from_spherely([None, spherely.create_point(1, 1)])
...
def data_for_sorting():
    ...
    A < B < C
    """
    return GeographyArray.from_shapely([Point(0, 1), Point(1, 1), Point(0, 0)])
...
def data_missing_for_sorting():
    ...
    A < B and NA missing.
    """
    return GeographyArray.from_shapely([Point(1, 2), None, Point(0, 0)])
...
def data_for_grouping():
    ...
    """
    return GeographyArray.from_shapely(
        [
...
class TestDtype(extension_tests.BaseDtypeTests):
    ...
    def test_array_type_with_arg(self, data, dtype):
        assert dtype.construct_array_type() is GeographyArray

    def test_registry(self, data, dtype):
        s = pd.Series(np.asarray(data), dtype=object)
        result = s.astype("geometry[spherical]")
        assert isinstance(result.array, GeographyArray)
        expected = pd.Series(data)
        assert_series_equal(result, expected)


class TestInterface(extension_tests.BaseInterfaceTests):
    @pytest.mark.xfail(reason="Spherely doesn't support missing values")
